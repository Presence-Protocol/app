...
interface AlreadyMintedWarningProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function AlreadyMintedWarning({ isOpen, onClose }: AlreadyMintedWarningProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{backgroundColor: 'rgba(0,0,0,0.7)'}}>
      <div className="bg-white rounded-xl shadow p-6 w-full max-w-md border-black border-2 shadow-black">
        ...
        <p className="text-sm text-gray-500 mb-6">
          You have already minted a Presence for this event. Each address can only mint one Presence per event.
        </p>
        <button
          onClick={onClose}
